...
const testimonials = [
  { quote: "We went from fragile, manual releases to a deployment process the whole team could trust.", role: "Technical founder", company: "Early-stage SaaS" },
  { quote: "The biggest change was clarity. Every infrastructure decision was documented, prioritized, and shipped without adding meetings.", role: "Head of Engineering", company: "Growing product team" },
  { quote: "ShipOps gave us senior platform judgment exactly when we needed it—without a months-long hiring process.", role: "Product leader", company: "Cloud-native company" },
];
const recommendations = [
  { plan: "Launch", signal: "Choose if you are…", title: "Building the foundation", copy: "You need reliable deployments, a clean cloud baseline, and monitoring before infrastructure becomes a full-time job." },
  { plan: "Scale", signal: "Best for most teams", title: "Shipping every week", copy: "Your product is growing, environments are multiplying, and developers need a dependable internal platform." },
  { plan: "Mission", signal: "Choose if you are…", title: "Operating at high stakes", copy: "Reliability, multi-cloud complexity, and faster incident response directly affect revenue or customer trust." },
];
...
export default function Home() {
  ...
  return <main>
    ...
    <header className="nav shell"><a className="brand" href="#top"><span className="brand-mark">S</span> SHIPOPS<span className="dot">.</span></a><nav><a href="#services">Services</a><a href="#scope">Scope</a><a href="#process">Process</a><a href="#stories">Stories</a><a href="#pricing">Pricing</a><a href="#faq">FAQ</a></nav><a className="nav-cta" href="#pricing">View plans <span>↗</span></a></header>
    ...
    <section className="stories shell" id="stories"><div className="stories-head"><div><p className="eyebrow"><span /> What good DevOps feels like</p><h2>LESS FRICTION.<br /><em>MORE SHIPPING.</em></h2></div><p className="sample-note">Sample testimonial copy · Replace with verified client quotes before publishing.</p></div><div className="testimonial-grid">{testimonials.map(({ quote, role, company }, index) => <figure key={quote} className={index === 1 ? "testimonial featured-quote" : "testimonial"}><span className="quote-mark" aria-hidden="true">“</span><blockquote>{quote}</blockquote><figcaption><span className="avatar" aria-hidden="true">{role.split(" ").map(word => word[0]).join("")}</span><span><strong>{role}</strong><small>{company}</small></span></figcaption></figure>)}</div><div className="recommendations"><div className="recommendation-intro"><p className="eyebrow"><span /> Our recommendation</p><h3>START WHERE THE<br />PRESSURE IS.</h3><p>Pick the capacity that matches today’s operational load. You can change plans as the platform evolves.</p></div>{recommendations.map(({ plan, signal, title, copy }) => <article key={plan}><p className="recommendation-signal">{signal}</p><span className="recommended-plan">{plan}</span><h4>{title}</h4><p>{copy}</p><a href="#pricing">See {plan} <span>↗</span></a></article>)}</div></section>
